...
export default function MockDraftsView({
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-white">Mock Drafts</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#2a3a4e] text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                  <th className="px-4 py-3 w-16">Pick</th>
              ...
              <tbody className="divide-y divide-[#2a3a4e]/50">
                {picks.map((pk, i) => (
                  <tr key={i} className="board-row">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-white">{pk.team}</td>
                    <td className="px-4 py-3">
                      {pk.slug ? (
                        <Link href={`/player/${pk.slug}`} className="text-sm font-semibold text-white hover:text-orange-400 transition-colors">
                          {pk.player}
                        </Link>
                      ) : (
                        <span className="text-sm text-white">{pk.player}</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <PositionBadge position={pk.position} />
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-400">{pk.college}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <thead>
            <tr className="border-b border-[#2a3a4e] text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              ...
              {compareSources.map((src) => (
                <th key={src} className="px-3 py-3 min-w-[180px]">
                  <div>{src}</div>
                  {mockDates[src] && (
